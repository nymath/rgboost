...
# Define a custom base estimator
class MyEstimator:
    def __init__(self):
    ...
    def _get_kernel(self, X, Y=None, nystroem_kernel=False):
        ...
        if (self.theta is None):
            if (self.n_neighbors == "inf") | (
                    self.n_neighbors == np.inf):  ##special case: chose theta such that it equals the average distance to the farest neighbor
            ...
            if not self.n_neighbors is None:
                if self.kernel == "GW":  ##Choose theta such that on average every point has n_neighbors non-zero entries
                    ...
                    dists, neighs = kdt.query(X,
                                              self.n_neighbors + 1)  ##get distance to n_neighbors+1 nearest neighbors (incl. point itself)
                    self.theta = np.mean(dists[:,
                                         self.n_neighbors])  ##calculate average distance to n_neighbors'th nearest neighbor (only true neighbors excl. point itself)
            if self.kernel == "rbf": self.theta = self.theta / (
                        self.range_adjust ** 0.5)  ##range_adjust=3 (4.6) correlation should drop to 5% (1%) at distance = theta
        ...
        if self.sparse:
            ...
            if self.solve_kernel is None:
                self.dual_coef_ = _solve_cholesky_kernel(self.kernel_mat, y, alpha,
                                                         sample_weight,
                                                         copy=True)  ##Need to copy since for the weighted case, the matrix gets modified
            else:
